...
from polycode.errors import OpencodeBinaryNotFoundError, OpencodeStartupError
...
async def test_raises_binary_not_found_when_opencode_missing(process):
    with patch("polycode.providers.opencode.process.shutil.which", return_value=None):
...
async def test_raises_startup_error_when_health_check_fails(process):
    with patch("polycode.providers.opencode.process.shutil.which", return_value="/usr/bin/opencode"):
